...
signal_keywords = [
    "founder", "building", "startup", "intern", "ai",
    "artificial intelligence", "ml", "machine learning", "engineering", "csun",
    "vc", "investing", "hustling", "launched", "operator", "automation"
]
intro_phrases = [
    "i'm", "i am", "my name is", "csun", "major", "studying", "building"
]
...
async def on_ready():
    ...
    if channel:
        ...
        async for message in channel.history(limit=100):
            ...
            if any(keyword in content for keyword in signal_keywords) or any(
                    phrase in content for phrase in intro_phrases):
                await message.add_reaction("✅")
                verified_role = discord.utils.get(message.guild.roles,
                                                  name=VERIFIED_ROLE)
                unverified_role = discord.utils.get(message.guild.roles,
                                                    name=UNVERIFIED_ROLE)
                if verified_role and verified_role not in message.author.roles:
                    await message.author.add_roles(verified_role)
                    await channel.send(
                        f"(Backfill) Welcome {message.author.mention} — verified as an operator. 🧠"
                    )
...
async def on_member_join(member):
    unverified_role = discord.utils.get(member.guild.roles,
                                        name=UNVERIFIED_ROLE)
    if unverified_role:
...
async def on_message(message):
    ...
    if any(keyword in content
           for keyword in signal_keywords) or any(phrase in content
        ...
        if verified_role and verified_role not in message.author.roles:
            ...
            await message.channel.send(
                f"Welcome {message.author.mention} — verified as an operator. 💰💰💰"
            )
